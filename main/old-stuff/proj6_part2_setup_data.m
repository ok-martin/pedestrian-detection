function imdb = proj6_part2_setup_data(prog, averageImage)
...
total_images = total_cats*num_train_per_category + total_cats * num_test_per_category;
...
for set = 1:length(sets)
    for category = 1:length(categories)
        cur_path = fullfile( imgs_path, categories{category}, sets{set});
        cur_images = dir( fullfile( cur_path, fileExtn) );
